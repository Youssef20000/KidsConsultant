...
$("#download").on('click', function () {
    html2canvas(document.querySelector("#imageDIV")).then(canvas => {
        let data = canvas.toDataURL('image/jpg').replace(/^data:image\/png/, "data:application/octet-stream");
        $("#download").attr("download", "image.png").attr("href", data).trigger('click');
    });
});

/* ************************************************************** */
let leftDays = $(".number .value").attr('value')/30;
var options = {
    value: leftDays,
    size: 200,
    startAngle: -Math.PI,
